...
original_author="Zwitschi"
original_name="opendata_collector"
original_urlname="opendata-collector"
original_description="Awesome opendata_collector created by Zwitschi" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/opendata_collector "${name}"
...
then
    ...
    echo "then run 'opendata_collector' or 'python -m opendata_collector'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'opendata_collector' or 'python -m opendata_collector' or 'poetry run opendata_collector'"
fi
