...
- 🎯 Entry points to execute your program using `python -m <opendata_collector>` or `$ opendata_collector` with basic CLI argument parsing.
...
---
# opendata_collector Flask Application

Awesome opendata_collector created by Zwitschi
...
```bash
git clone https://github.com/Zwitschi/opendata-collector opendata_collector
cd opendata_collector
make install
```
...
```bash
pip install opendata_collector
```
...
```bash
opendata_collector create-db   # run once
opendata_collector populate-db  # run once (optional)
opendata_collector add-user -u admin -p 1234  # ads a user
opendata_collector run
```
